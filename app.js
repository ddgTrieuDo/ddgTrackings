...
function taskManagedProjectMembers(pid){return projectMembers(pid).map(m=>m.user_id)}
function myTaskProjectIds(){const memberPids=state.members.filter(m=>m.user_id===state.profile?.id).map(m=>m.project_id);const assigneePids=state.tasks.filter(t=>state.assignees.some(a=>a.task_id===t.id&&a.user_id===state.profile?.id)).map(t=>t.project_id);return [...new Set([...memberPids,...assigneePids].filter(Boolean))]}
function renderTasks(){const el=$('#page-tasks');const items=[['mine','My Tasks']];if(canManageAnyProject())items.push(['manage','Task Management']);if(!items.some(x=>x[0]===state.tasksTab))state.tasksTab='mine';el.innerHTML=tabs(items,state.tasksTab)+`<div id="tasksBody"></div>`;hookTabs(el,v=>{state.tasksTab=v;if(v!=='manage')state.selectedManagedTaskId=''});if(state.tasksTab==='mine')renderMyTasks();else renderTaskManagement()}
function renderMyTasks(){const projectIds=myTaskProjectIds();const projects=projectIds.map(pById).filter(Boolean).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||'')));if(!projects.some(p=>p.id===state.myTasksProjectId))state.myTasksProjectId=projects[0]?.id||'';const pid=state.myTasksProjectId||'';const projectFilter=(state.myTasksProjectFilter||'').toLowerCase();const taskFilter=(state.myTasksTaskFilter||'').toLowerCase();const visibleProjects=projects.filter(p=>!projectFilter||taskSearchBlob({project_id:p.id,name:p.name,status:'',phase:'',trade:'',level:'',building:'',system_name:'',planned_delivery:'',actual_delivery:'',id:''}).includes(projectFilter));const tasks=state.tasks.filter(t=>t.project_id===pid).filter(t=>!taskFilter||taskSearchBlob(t).includes(taskFilter)).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||'')));const summaryProject=pById(pid);const hours=tasks.reduce((s,t)=>s+taskHours(t.id),0);$('#tasksBody').innerHTML=`<div class="app-task-section"><h2>My Tasks</h2><div class="app-task-layout"><div class="card task-project-column"><div class="field"><label>Select project</label><input id="myProjectSearch" placeholder="Search project" value="${esc(state.myTasksProjectFilter||'')}"></div><div class="task-project-list">${visibleProjects.map(p=>`<button class="task-project-item ${p.id===pid?'active':''}" data-my-project="${p.id}">${esc(p.name)}</button>`).join('')||'<div class="muted">No projects assigned.</div>'}</div><div class="muted small">Create/edit Projects in Jobs > Project Setup; manage Tasks in Tasks > Task Management</div></div><div class="task-cards-column"><div class="task-cards-head"><div><div class="section-title" style="margin-bottom:4px">Track on task</div><div class="muted small">${summaryProject?`${esc(summaryProject.name)} • ${tasks.length} tasks • ${fmt(hours)} h tracked`:'Select a project'}</div></div><div class="field grow"><label>Search task</label><input id="myTaskSearch" placeholder="Search task" value="${esc(state.myTasksTaskFilter||'')}"></div></div><div class="task-card-stack">${tasks.map(t=>{const as=taskAssigneeNames(t.id),mine=state.assignees.some(a=>a.task_id===t.id&&a.user_id===state.profile?.id);return `<div class="task-card ${mine?'mine':''}"><div class="task-card-top"><div><div class="task-card-name">${esc(t.name||'—')}</div><div class="task-card-meta">${esc(t.phase||'No phase')} • ${esc(t.trade||'No trade')} • ${fmt(taskHours(t.id))} h</div></div><span class="pill">${esc(t.status||'Ready')}</span></div><div class="task-card-grid"><div><b>System</b><br>${esc(t.system_name||'—')}</div><div><b>Level</b><br>${esc(t.level||'—')}</div><div><b>Building</b><br>${esc(t.building||'—')}</div><div><b>% Complete</b><br>${taskProgressValue(t)}%</div></div><div class="task-card-foot"><span class="muted">Assignees: ${esc(as.join(', ')||'—')}</span><span class="muted">People: ${as.length}</span></div></div>`}).join('')||'<div class="card"><div class="muted">No tasks found for this project.</div></div>'}</div></div></div></div>`;$$('[data-my-project]').forEach(b=>b.onclick=()=>{state.myTasksProjectId=b.dataset.myProject;renderMyTasks()});const psearch=$('#myProjectSearch');if(psearch)psearch.oninput=e=>{state.myTasksProjectFilter=e.target.value;renderMyTasks()};const tsearch=$('#myTaskSearch');if(tsearch)tsearch.oninput=e=>{state.myTasksTaskFilter=e.target.value;renderMyTasks()}}
...
function renderTaskManagement(){const allowed=managedProjects();if(!allowed.some(p=>p.id===state.taskManageProjectId))state.taskManageProjectId=allowed[0]?.id||'';const pid=state.taskManageProjectId||'';const project=pById(pid);const members=projectMembers(pid);const rows=state.tasks.filter(t=>t.project_id===pid);const opts=allowed.map(p=>`<option value="${p.id}" ${p.id===pid?'selected':''}>${esc(p.name)}</option>`).join('');$('#tasksBody').innerHTML=`<div class="app-task-section"><h2>Task Management</h2><div class="field task-project-picker"><label>Project</label><select id="taskProject">${opts}</select></div><div class="muted" style="margin:12px 0 18px">${project?`${esc(project.name)}: ${members.length} members, ${rows.length} task.`:'You do not manage any projects yet.'}</div><div class="card management-board"><div class="management-actions"><button id="editTaskBtn" ${state.selectedManagedTaskId?'':'disabled'}>Edit Task</button><button id="deleteTaskBtn" class="danger" ${state.selectedManagedTaskId?'':'disabled'}>Delete Task</button><button id="newTaskBtn" class="primary" ${pid?'':'disabled'}>+ New Task</button></div><div id="taskGrid"></div><div id="taskSelectionInfo" class="muted small management-note"></div><div class="muted small management-note">Direct edit: click Phase or Assignees to open a drop-down; double-click Status to change status; double-click Task name to edit.</div></div></div>`;const s=$('#taskProject');if(s)s.onchange=()=>{state.taskManageProjectId=s.value;state.selectedManagedTaskId='';renderTaskManagement()};const newBtn=$('#newTaskBtn');if(newBtn)newBtn.onclick=()=>taskModal(pid);const editBtn=$('#editTaskBtn');if(editBtn)editBtn.onclick=()=>{const task=tById(state.selectedManagedTaskId);if(task)taskModal(pid,task)};const delBtn=$('#deleteTaskBtn');if(delBtn)delBtn.onclick=()=>state.selectedManagedTaskId&&archiveTask(state.selectedManagedTaskId);drawTaskGrid(pid);updateTaskManagementButtons(pid)}
function drawTaskGrid(pid){const rows=state.tasks.filter(t=>t.project_id===pid).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||'')));const selected=rows.find(t=>t.id===state.selectedManagedTaskId)||null;const info=$('#taskSelectionInfo');if(info)info.textContent=selected?`Task: ${selected.name}  •  Status: ${selected.status||'Ready'}  •  Tracked: ${fmt(taskHours(selected.id))} h  •  Assigned: ${taskAssigneeNames(selected.id).length} people`:'Click Phase or Assignees to edit directly. Double-click Status to change it.';$('#taskGrid').innerHTML=`<div class="table-wrap"><table class="table task-management-table"><thead><tr><th>Task</th><th>Status</th><th>Phase</th><th>Trade</th><th>Level</th><th>Building</th><th>System</th><th>% Complete</th><th>Tracked Hours</th><th>Assignees</th><th>People</th></tr></thead><tbody>${rows.map(t=>{const as=taskAssigneeNames(t.id);return `<tr class="${t.id===state.selectedManagedTaskId?'selected':''}" data-task-row="${t.id}"><td>${esc(t.name||'—')}</td><td>${esc(t.status||'Ready')}</td><td>${esc(t.phase||'Select phase...')}</td><td>${esc(t.trade||'—')}</td><td>${esc(t.level||'—')}</td><td>${esc(t.building||'—')}</td><td>${esc(t.system_name||'—')}</td><td>${taskProgressValue(t)}%</td><td>${fmt(taskHours(t.id))} h</td><td>${esc(as.join(', ')||'Select people...')}</td><td>${as.length}</td></tr>`}).join('')||'<tr><td colspan="11" class="muted">No tasks in this project.</td></tr>'}</tbody></table></div>`;$$('[data-task-row]').forEach(r=>{r.onclick=()=>{state.selectedManagedTaskId=r.dataset.taskRow;drawTaskGrid(pid);updateTaskManagementButtons(pid)};r.ondblclick=e=>{const task=tById(r.dataset.taskRow);if(task)taskModal(pid,task)}})}
...
function renderAbout(){$('#page-about').innerHTML=`<div class="card" style="max-width:900px"><div class="section-title">DDG Tracking Web</div><p>Web management portal synchronized with the DDG Tracking Windows desktop agent through Supabase.</p><p><b>Shared management features:</b> Dashboard, Project Setup/List/Time, Change Orders, My Tasks, Task Management, Production, Screenshots, Attendance/Leave/Schedules, Team Monitor and Employee Management.</p><p><b>Desktop agent:</b> Start/Stop tracking, keyboard/mouse activity, active applications, automatic screenshots, offline/background tracking and system tray operation.</p><p class="muted">Version Web 4.17 Task Tabs App Match</p></div>`}
