...
async function archiveTask(id){if(!confirm('Delete this Task? Tracking history is kept.'))return;const {error}=await sb.from('tasks').update({active:false}).eq('id',id);if(error)alert(error.message);else loadAll()}
