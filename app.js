...
function renderProjectSetup(){const body=$('#jobsBody');const allowed=managedProjects(),pid=allowed[0]?.id||'';const opts=allowed.map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('');body.innerHTML=`<div class="toolbar"><div class="field grow"><label>Project</label><select id="setupProject">${opts}</select></div>${roleIsAdmin()?'<button id="newProject">+ New Project</button><button id="renameProject">Rename</button><button id="deleteProject" class="danger">Delete</button>':''}<button id="saveSetup" class="primary" ${pid?'':'disabled'}>Save Project Setup</button></div><div id="setupForm"></div>`;const sel=$('#setupProject');sel.onchange=()=>drawSetupForm(sel.value);drawSetupForm(sel.value);if(roleIsAdmin()){ $('#newProject').onclick=()=>projectModal();$('#renameProject').onclick=()=>projectModal(pById(sel.value),true);$('#deleteProject').onclick=()=>archiveProject(sel.value)}$('#saveSetup').onclick=()=>saveProjectSetup(sel.value)}
...
function renderTasks(){const el=$('#page-tasks');const items=[['mine','My Tasks']];if(canManageAnyProject())items.push(['manage','Task Management']);if(roleIsManager())items.push(['all','All Tasks']);if(!items.some(x=>x[0]===state.tasksTab))state.tasksTab='mine';el.innerHTML=tabs(items,state.tasksTab)+`<div id="tasksBody"></div>`;hookTabs(el,v=>state.tasksTab=v);if(state.tasksTab==='mine')renderMyTasks();else if(state.tasksTab==='manage')renderTaskManagement();else renderAllTasks()}
...
function renderAllTasks(){const rows=state.tasks;$('#tasksBody').innerHTML=`<div class="toolbar"><div class="field grow"><label>Search</label><input id="allTaskSearch" placeholder="Search tasks..."></div></div><div class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Customer</th><th>Project</th><th>Task</th><th>Member</th><th>Hours Used</th><th>Status</th><th>Created Date</th><th>Planned Delivery</th><th>Task Deadline</th></tr></thead><tbody>${rows.map(t=>{const p=pById(t.project_id),members=state.assignees.filter(a=>a.task_id===t.id).map(a=>nameOf(a.user_id)).join(', ');return`<tr data-all-task-row data-search="${esc([p?.customer,p?.name,t.name,members,t.status].join(' ').toLowerCase())}"><td>${esc(p?.customer||'—')}</td><td>${esc(p?.name||'—')}</td><td>${esc(t.name)}</td><td>${esc(members||'—')}</td><td>${fmt(taskHours(t.id))}</td><td>${esc(t.status||'Ready')}</td><td>${esc(String(t.created_at||'').slice(0,10)||'—')}</td><td>${esc(t.planned_delivery||'—')}</td><td>${esc(t.actual_delivery||'—')}</td></tr>`}).join('')}</tbody></table></div></div>`;$('#allTaskSearch').oninput=e=>{const q=e.target.value.toLowerCase();$$('[data-all-task-row]').forEach(r=>r.classList.toggle('hidden',q&&!r.dataset.search.includes(q)))}}
function renderTaskManagement(){const allowed=managedProjects(),pid=allowed[0]?.id||'',opts=allowed.map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('');$('#tasksBody').innerHTML=`<div class="toolbar"><div class="field"><label>Project</label><select id="taskProject">${opts}</select></div><button id="newTask" ${pid?'':'disabled'}>+ New Task</button></div><div id="taskGrid"></div>`;const s=$('#taskProject');s.onchange=()=>drawTaskGrid(s.value);$('#newTask').onclick=()=>taskModal(s.value);drawTaskGrid(s.value)}
